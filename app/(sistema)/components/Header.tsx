...
export default function Header() {
  ...
  useEffect(() => {
    const sessaoCookie = getCookie('sessao_admin');
    setEstaLogado(!!sessaoCookie);

    try {
      const sessao = JSON.parse(sessaoCookie as string)
      setRole(sessao.role)
      setUserId(sessao.userId)
      setUserNome(sessao.userNome)
    } catch {
      setRole(null)
    }
  ...
  const linkClasse = (rota: string) =>
    `relative py-1 transition-colors duration-200 ${pathname === rota ? 'text-amber-400' : 'text-slate-200 hover:text-amber-400'
    }`;
        ...
        <div className="px-4 pt-3 pb-4 flex flex-col gap-1 text-[15px]">
          ...
          {(role === 'ADMIN' || role === 'USER') && (
            <>
              ...
              <div className={`${dropdownMobileAberto ? 'max-h-60' : 'max-h-0'} overflow-hidden transition-all duration-200 pl-3 border-l border-amber-400/30 ml-3`}>
                <Link href={"/administracao/colaboradores/" + userId} onClick={fecharMenu} className="block px-3 py-2 text-sm text-slate-300 hover:text-amber-400">
                  Meu Perfil
                </Link>
                ...
                {role === 'ADMIN' && (
                  <>
                    ...
                    <Link href="/administracao/produtos" onClick={fecharMenu} className="block px-3 py-2 text-sm text-slate-300 hover:text-amber-400">
                      Produtos
                    </Link>
                  </>
                )}
